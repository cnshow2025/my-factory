...
const AR = { c_factors:2.5522, c_cum:2.5522, c_depth:1.3965, c_late:1.3965, c_eng:1.3965,
             ...
             c_fleet:1.4554, c_rematch:1.4554, c_switch:2.4204 };
...
/* ===== 3 模型與校準 ===== */
s = pres.addSlide();
head(s, "02", "模型與校準", "模型不是假設出來的：用現場三項實測值反推參數，三項同時對上才開始分析");
s.addTable([
...
img(s, "c_ded", 0.62, 1.50, 5.2);          // 底緣 ≈ 5.46
img(s, "c_lever", 6.30, 1.50, 6.4);        // 底緣 ≈ 5.51
card(s, 0.62, 5.62, 6.0, 1.58, "專用化：現況已接近最佳", [
  "5 台彈性機扛下所有換 Kit，現況 39 次／日",
  "完全專用再省 70 分／台／日，但彈性歸零",
  "急單無處插隊，逾期反而由 5.0 升到 5.5 批"
], TEAL);
card(s, 6.75, 5.62, 5.92, 1.58, "槓桿：換程式才是大宗", [
...
img(s, "c_switch", 1.47, 1.40, 10.4);      // 底緣 ≈ 5.70
card(s, 0.62, 5.78, 6.0, 1.30, "怎麼量（用現場的實績，不用模擬）", [
  "（改機分鐘 ＋ 等工程師分鐘）÷（機台數 × 當班分鐘）",
  "模擬器的跑分表直接有這兩欄，現場的機台稼動報表也有"
], PRIMARY);
card(s, 6.75, 5.78, 5.96, 1.30, "怎麼判斷", [
  "超過 35% → 改用「省改機優先」，10 組情境全對",
  "低於 30% → 改用「大批給快機」；30～35% 之間兩者差不到 15%"
], GOOD);
T(s, "註　兩項要合計才準：5 台機那三組是改機本身吃掉 35%（等人只有 0～2%），25 台機 200 批 3 人那組反過來是等人吃掉 36%（改機只有 10%）——只看其中一項都會判錯。量測時用現行的「追交期」排程當基準。",
  { x:0.62, y:7.10, w:12.09, h:0.19, fontSize:9.5, color:MUTED });
...
T(s, "但第 1 項有前提：省改機是「改機＋等工程師吃掉機台時間超過 35%」時的最佳解；低於 30% 就該改用「大批給快機」（見第 12 頁）。班別與機隊狀況不同，規則就該不同。",
